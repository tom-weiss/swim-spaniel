...
type AnchorDropPhase = "telegraph" | "falling";
type KomodoBossPhase = "entering" | "hovering" | "exiting";
type BehaviorState =
  | { kind: "anchorFall"; phase: AnchorDropPhase; phaseMs: number }
  | { kind: "mantaCurrent"; pushDirection: 1 | -1; pushCooldownMs: number }
  | {
    kind: "fishSchool";
    anchorLane: number;
    laneSpan: number;
  ...
  | { kind: "komodoBoss"; phase: KomodoBossPhase; phaseMs: number; throwCooldownMs: number };
...
export interface RescueEffect {
  kind: "spaniel-rescue" | "creature-stunned" | "rescue-pop" | "venom-splash";
  x: number;
  y: number;
  ttlMs: number;
  maxTtlMs: number;
...
export interface GameSnapshot {
  ...
  score: number;
  speedLevel: number;
  spanielsRescued: number;
  levelSpanielsRescued: number;
  nextBossRescueGoal: number;
  isGameOver: boolean;
  playerX: number;
  playerY: number;
  playerJumpOffset: number;
  isPlayerImmortal: boolean;
  playerImmortalMs: number;
  ...
  effects: ReadonlyArray<RescueEffect>;
}
...
export class SwimSpanielGame {
  ...
  private spanielsRescued = 0;
  private levelSpanielsRescued = 0;
  private gameOver = false;
  private komodoBossActive = false;
  private victory = false;
  private entities: Entity[] = [];
  private effects: RescueEffect[] = [];
  private rng: () => number;
  ...
  private static readonly siltSlowDurationMs = 900;
  private static readonly currentStreamBoostDurationMs = 1400;
  private static readonly anchorTelegraphDurationMs = 650;
  private static readonly mantaCurrentPushIntervalMs = 180;
  private static readonly komodoBossEnterY = 58;
  private static readonly komodoBossHoverDurationMs = 20000;
  private static readonly komodoBossEnterSpeed = 2.05;
  private static readonly komodoBossExitSpeed = 1.95;
  private static readonly komodoBossLaneMoveMs = 240;
  private static readonly komodoBossThrowMinMs = 820;
  private static readonly komodoBossThrowMaxMs = 1500;
  private static readonly komodoBossThrowMinFloorMs = 260;
  private static readonly komodoBossThrowRangeFloorMs = 180;
  private static readonly komodoThrowLevelReductionMs = 60;
  private static readonly komodoBossBonusScore = 1800;
  ...
  private static readonly venomCloudSpeed = 2.8;
  private static readonly venomCloudWidthScale = 0.34;
  private static readonly venomCloudHeight = 16;
  private static readonly maxSiltStackMs = 5400;
  private static readonly maxCurrentStackMs = 5600;
  ...
  private static readonly siltLaneSwitchCooldownMs = 280;
  private static readonly siltJumpCooldownMs = 920;
  private static readonly currentLaneSwitchCooldownMs = 80;
  private static readonly currentJumpCooldownMs = 360;
  private static readonly siltScrollSpeedMultiplier = 0.72;
  private static readonly currentScrollSpeedMultiplier = 1.35;
  private static readonly fishSchoolObstacleId = "fish-school";
  private static readonly fishSchoolMinChildren = 3;
  private static readonly fishSchoolBaseChildren = 3;
  private static readonly fishSchoolMaxChildren = 16;
  private static readonly fishSchoolLeaderWidthScale = 0.56;
  private static readonly fishSchoolFollowerWidthScale = 0.5;
  private static readonly fishSchoolLeaderHeight = 30;
  private static readonly fishSchoolFollowerHeight = 28;
  private static readonly fishSchoolSegmentSpacingPx = 16;
  private static readonly fishSchoolWavePeriodMs = 1080;
  private static readonly fishSchoolWaveOffsetMs = 170;
  private static readonly fishSchoolSpeedVariance = 0.16;
  private static readonly victoryLevel = 10;
  ...
  private currentSpawnIntervalMs(): number {
    ...
    const baselineSpawnInterval = Math.max(
      SwimSpanielGame.minSpawnIntervalMs,
      SwimSpanielGame.levelOneBaseSpawnIntervalMs - (effectiveLevel - 1) * SwimSpanielGame.spawnIntervalLevelStepMs
    );
    if (this.levelTransitionBoostMs > 0) {
      return Math.max(
        SwimSpanielGame.levelTransitionSpawnFloorMs,
        Math.min(SwimSpanielGame.levelTransitionSpawnIntervalMs, baselineSpawnInterval - 80)
      );
    }
  ...
  private fishSchoolFollowersForLevel(level = this.speedLevel): number {
  ...
  private fishSchoolLaneSpanForLevel(level = this.speedLevel): number {
    return level >= 7 ? 2 : 1;
  }

  private clampFishSchoolAnchorLane(lane: number, laneSpan: number): number {
  ...
  private computeFishSchoolLane(anchorLane: number, laneSpan: number, phaseMs: number, wavePeriodMs: number): number {
  ...
  private spawnFishSchool(
    ...
    for (let fishIndex = 0; fishIndex < totalFish; fishIndex += 1) {
      ...
      const lane = this.computeFishSchoolLane(
        anchorLane,
        laneSpan,
        waveSeedMs + phaseOffsetMs,
        SwimSpanielGame.fishSchoolWavePeriodMs
      );
  ...
  private spawnKomodoVenomCloud(komodo: Entity): void {
    const maxSpread = Math.max(0, Math.min(2, Math.floor((this.speedLevel - 3) / 3)));
    const lanes = new Set<number>([this.playerLane]);
  ...
  private rollKomodoThrowCooldownMs(): number {
    const effectiveLevel = Math.min(this.speedLevel, SwimSpanielGame.maxSpeedLevel);
  ...
  private bossRescueGoalForLevel(level: number): number {
  ...
  private tickEntityBehaviorState(entity: Entity, deltaMs: number): void {
    ...
    if (behavior.kind === "fishSchool") {
      ...
      const lane = this.computeFishSchoolLane(
        behavior.anchorLane,
        behavior.laneSpan,
        behavior.phaseMs + behavior.phaseOffsetMs,
        behavior.wavePeriodMs
      );
      entity.lane = lane;
      return;
    }
  ...
  private applyMantaCurrentPushes(): void {
    ...
    for (const entity of this.entities) {
      ...
      if (entity.type === "silt-patch") {
        ...
        if (id < 0 || !this.touchingSurfaceEntityIds.has(id)) {
          this.siltSlowMs = Math.min(
            SwimSpanielGame.maxSiltStackMs,
            this.siltSlowMs + SwimSpanielGame.siltSlowDurationMs
          );
        }
        survivors.push(entity);
        continue;
      ...
      if (entity.type === "current-stream") {
        ...
        if (id < 0 || !this.touchingSurfaceEntityIds.has(id)) {
          this.currentBoostMs = Math.min(
            SwimSpanielGame.maxCurrentStackMs,
            this.currentBoostMs + SwimSpanielGame.currentStreamBoostDurationMs
          );
        }
        survivors.push(entity);
        continue;
  ...
  private isFishSchoolEntity(entity: Entity): boolean {
    return entity.type === "fish-school-leader" || entity.type === "fish-school-follower";
  }

  private isSameFishSchoolGroup(first: Entity, second: Entity): boolean {
  ...
  private spawnRescueEffect(x: number, y: number, kind: RescueEffect["kind"]): void { this.effects.push({ kind, x, y, ttlMs: 300, maxTtlMs: 300 }); }

  private spawnRescueBubbles(entity: Entity): void {
  ...
  public spawnFishSchoolDebug(followerCount?: number): void {
    if (this.gameOver) {
      return;
    }
  ...
  private maybeMoveEntityLane(entity: Entity, deltaMs: number): void {
    if (entity.type !== "shark"
      ...
      && entity.type !== "black-spaniel"
      && entity.type !== "komodo"
      && entity.type !== "manta-current") return;
...
export class PixelRenderer {
  ...
  public render(snapshot: GameSnapshot): void {
    ...
    for (const entity of snapshot.entities) {
      ...
      } else if (entity.type === "coral") drawCoral(this.ctx, entity.x, entity.y, entity.obstacleId, entity.width);
      ...
      } else if (entity.type === "shark") drawShark(this.ctx, entity.x, entity.y, entity.obstacleId);
      else if (entity.type === "fish-school-leader") drawFish(this.ctx, entity.x, entity.y, "#fb7185", "#fecdd3");
      ...
      else if (entity.type === "jellyfish") drawJellyfish(this.ctx, entity.x, entity.y);
      else if (entity.type === "spaniel") drawSpaniel(this.ctx, entity.x, entity.y, snapshot.sideObstacleOffsetY + entity.x, "brown", entity.obstacleId);
      else if (entity.type === "black-spaniel") drawSpaniel(this.ctx, entity.x, entity.y, snapshot.sideObstacleOffsetY + entity.x, "black", entity.obstacleId);
      else if (entity.type === "rescue-bubbles") drawRescueBubbles(this.ctx, entity.x, entity.y);
      else {
    ...
    for (const effect of snapshot.effects) drawRescueEffect(this.ctx, effect);
  ...
  private drawReefEdges(offsetY: number): void {
  ...
  private computeReefEdgeDecorations(offsetY: number): SideDecorationPlacement[] {
    ...
    const tryPlace = (kind: "coral" | "reef-rock", x: number, y: number): void => {
  ...
  private sideDecorationBounds(kind: "coral" | "reef-rock", x: number, y: number): Bounds {
    if (kind === "coral") {
...
function drawCoral(ctx: CanvasRenderingContext2D, x: number, y: number, obstacleId = "staghorn-coral", width = 22): void {
...
function drawReefRock(ctx: CanvasRenderingContext2D, x: number, y: number, obstacleId = "volcanic-rock-cluster", width = 22): void {
...
function drawRescueBubbles(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawSpaniel(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  animationSeed = 0,
  coat: "brown" | "black" = "brown",
  obstacleId = "floating-life-ring"
): void {
  ...
  if (obstacleId === "drifting-rescue-rope" || obstacleId === "tangled-spaniel") {
    ...
    if (obstacleId === "tangled-spaniel") ctx.fillRect(x + 2, y + 17, 12, 2);
  }
  ctx.fillStyle = "#111827";
  ctx.fillRect(x + 18, y + 9, 1, 1);
  ctx.fillRect(x + 21, y + 10, 1, 1);
}
function drawSiltPatch(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawCurrentStream(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawKelpBed(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawReefTrench(ctx: CanvasRenderingContext2D, x: number, y: number, width: number): void {
...
function drawAnchorTelegraph(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawFallingAnchor(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawVenomCloud(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawDiver(ctx: CanvasRenderingContext2D, x: number, y: number, animationSeed = 0, boostOffset = 0): void {
  ...
  if (boosted) ctx.fillRect(x - 3, bodyY + 6, 5, 2);
}

function drawShark(ctx: CanvasRenderingContext2D, x: number, y: number, obstacleId = "reef-shark"): void {
  ...
  if (isHammerhead) ctx.fillRect(x + 17, y + 8, 7, 3);
...
function drawFish(ctx: CanvasRenderingContext2D, x: number, y: number, bodyColor: string, highlightColor: string): void {
...
const FISH_SCHOOL_COLORS: ReadonlyArray<{ body: string; helmet: string }> = [
...
function drawFishSchoolFollower(ctx: CanvasRenderingContext2D, x: number, y: number, paletteIndex: number): void {
...
function drawJellyfish(ctx: CanvasRenderingContext2D, x: number, y: number): void {
  ctx.fillStyle = "#c084fc";
...
function drawMantaCurrent(ctx: CanvasRenderingContext2D, x: number, y: number, pushDirection: 1 | -1): void {
  ctx.fillStyle = "#1e293b";
...
function drawKomodoShadow(ctx: CanvasRenderingContext2D, x: number, y: number, animationSeed = 0): void {
...
function drawKomodo(ctx: CanvasRenderingContext2D, x: number, y: number, animationSeed = 0): void {
...
function drawStunnedDiver(ctx: CanvasRenderingContext2D, x: number, y: number): void {
...
function drawBoostShadow(ctx: CanvasRenderingContext2D, x: number, y: number, jumpOffset: number): void {
...
function drawRescueShield(ctx: CanvasRenderingContext2D, x: number, y: number, immortalMs: number): void {
...
function drawRescueEffect(ctx: CanvasRenderingContext2D, effect: RescueEffect): void {
  if (effect.kind === "rescue-pop") {
